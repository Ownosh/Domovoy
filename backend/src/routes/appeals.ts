...
async function fetchWithParticipants(appealIds: number[]): Promise<object[]> {
    ...
    const [parts] = await pool.query<RowDataPacket[]>(
        `SELECT ap.appeal_id, ap.user_id, uap.apartment, uap.entrance, p.full_name AS display_name,
...
router.post("/:id/join", requireAuth, async (req: AuthRequest, res) => {
    ...
    try {
        ...
        try {
            await pool.execute(
                `INSERT INTO appeal_participants (appeal_id, user_id, apartment_id, anonymous, comment, photo_uri)
                 VALUES (?, ?, ?, ?, ?, ?)`,
                [appealId, userId, prof.apartmentId, anonymous ? 1 : 0, comment?.trim() || null, photoUri || null],
            );
        } catch (e: any) {
