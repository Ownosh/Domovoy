...
function normalizeFeedbackTagIds(raw: unknown): string[] | undefined {
    if (!Array.isArray(raw)) return undefined;
    const tags = raw.filter(
        (id): id is string => typeof id === "string" && VALID_RATING_FEEDBACK_TAGS.has(id),
    );
    return tags.length > 0 ? tags : undefined;
}

async function loadFeedbackTagIds(ratingId: number): Promise<string[] | undefined> {
    const [rows] = await pool.query<RowDataPacket[]>(
        `SELECT tag_id FROM environment_rating_feedback_tags
         WHERE rating_id = ? ORDER BY tag_id`,
        [ratingId],
    );
...
async function replaceFeedbackTags(ratingId: number, tagIds: string[] | undefined): Promise<void> {
    await pool.execute(`DELETE FROM environment_rating_feedback_tags WHERE rating_id = ?`, [ratingId]);
    if (!tagIds?.length) return;
    for (const tagId of tagIds) {
        await pool.execute(
            `INSERT INTO environment_rating_feedback_tags (rating_id, tag_id) VALUES (?, ?)`,
            [ratingId, tagId],
        );
    }
}
...
router.get("/my", requireAuth, async (req: AuthRequest, res) => {
    ...
    try {
        ...
        let sql = `SELECT id, month_key, courtyard_stars, entrance_stars, uk_stars,
        ...
        const feedbackTagIds = await loadFeedbackTagIds(Number(row.id));
...
router.post("/", requireAuth, async (req: AuthRequest, res) => {
    ...
    const normalizedTags = normalizeFeedbackTagIds(feedbackTagIds);
    ...
    try {
        ...
        const [[ratingRow]] = await pool.query<RowDataPacket[]>(
            `SELECT id FROM environment_ratings
             WHERE apartment_id = ? AND month_key = ?`,
            [apt.apartmentId, monthKey],
        );
        if (ratingRow) {
            await replaceFeedbackTags(Number(ratingRow.id), normalizedTags);
        }
